...
class ReadPanel extends React.Component {
  ...
  handleTouchEnd(e) {
    ...
    // 滑动 判断
    if (
      Math.abs(this.movingRecord.sumX) > 50 &&
      Math.abs(this.movingRecord.sumY) < 15
    ) {
      // 成立
      this.translate(e)
      // console.log("window.getSelection", window.getSelection(), e);
      // console.log('this.movingRecord', this.movingRecord)
      // console.log('core',
      //   core.calcOffset(this.movingRecord.startTarget, this.movingRecord.startX, this.movingRecord.startY)
      // );
    }
    ...
    let touchMove =
      Math.abs(this.movingRecord.sumX) < 15 &&
      Math.abs(this.movingRecord.sumY) < 15;
    if (touchTime > 800 && touchTime < 2000 && touchMove) {
      e.preventDefault();
      e.stopPropagation();
  ...
  handleRpHidden() {
    ...
    if (this.touchTarget.className.split(" ").includes("wrp-read-panel"))
      return;
      ...
      >
        ...
        <div id="wrp-rp-menu" style={this.state.menuPosition}>
          <div
            ...
            onClick={() => this.handleCloseRpMenu()}
          >
            <svg
          ...
          <div className="wrp-nav-item" onClick={() => this.handleRpShow()}>
            <svg
              className="wrp-nav-item-icon"
              fill="var(--t-fore-c)"
              viewBox="0 0 1024 1024"
